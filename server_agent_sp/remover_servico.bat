...
if !errorlevel! equ 0 (
    ...
    if !errorlevel! equ 0 (
        ...
        if !errorlevel! neq 0 (
            ...
            call :L "[AVISO] Encerrando o processo do servico..."
            :: Encerra apenas o PID deste servico — nunca todos os powershell.exe
            set SVCPID=
            for /f "tokens=3" %%a in ('sc queryex %SERVICE_NAME% ^| findstr /i "PID"') do set SVCPID=%%a
            if defined SVCPID if not "!SVCPID!"=="0" taskkill /f /pid !SVCPID! >nul 2>&1
            timeout /t 2 /nobreak >nul
        )
